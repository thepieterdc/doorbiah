...
const Branddeur = function (canvas, x, y, dx, dy) {
  ...
  let collideRect = function (head, c1, c2, c3, c4) {
    return head.x + c1 * head.width > self.x && head.x + c2 * head.width < self.x + self.width && head.y + c3 * head.height > self.y && head.y + c4 * head.height < self.y + self.height;
  };

  this.collides = function (head) {
    return collideRect(head, 0.8, 0.2, 1, 0) || collideRect(head, 0.9, 0.1, 0.2, 0.1) || collideRect(head, 1, 0, 0.7, 0.2);
  };
...
$(document).ready(function () {
  ...
  game.draw(); // show_alert('Gebuisd!', 'Je hebt gefaalt. Maar geen nood, je krijgt nog een herkansing. Het herexamen begint nu.');
